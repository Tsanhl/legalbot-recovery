# Answer guide from the Law folder (public copy; owner-approved)

Built 25 September 2026 from a full read of every marked script, formative,
general-feedback document, assessment-criteria document, writing guide and
OSCOLA 5 guide in `~/Desktop/Law`. The evidence and marks behind each rule are kept in a private, unpublished copy. Once approved, it replaces the
18-rule `owner-standards-2026-09-23.3` bundle, and Qwen receives a compact
version (section 9).

This is a drafting standard distilled from the owner's marked work. It is
not legal authority and does not guarantee a mark.

---


## 2. What separates 70+ from 60s and 50s

University band descriptors, with the differences highlighted:

| Skill | First (70+) | Upper second (60–69) | Lower second (50–59) |
| --- | --- | --- | --- |
| Knowledge | accurate, **detailed and comprehensive**; **real insight** into economic, social and political pressures | accurate knowledge of a **substantial majority**; an **appreciation** of pressures | accurate knowledge of **the majority**; **some** appreciation |
| Problem solving | **insightful** conclusions supported by **persuasive** argument, where doctrine is clear **and where it is disputed** | **reasonable** conclusions supported by legal argument | conclusions supported by legal argument |
| Research | retrieves **all** relevant primary **and secondary** material; **clear and defensible** opinion; referencing **correct** | **most** material; a **defensible** opinion; referencing correct | some material; a **reasoned** opinion; **some familiarity** with referencing |
| Analysis | ranks issues **persuasively**; **persuasive synthesis in context**; defends personal opinion on reform **persuasively** | ranks issues **justifiably**; a **good** synthesis; opinion defended **in a reasoned manner** | ranks issues **in a reasoned manner**; **some** synthesis; **offers** opinions |
| Communication | complex ideas with **economy, clarity and confidence** | all but the most complex ideas | straightforward ideas |

the Law & Medicine convenor's summary of the same scale: 3rd understands others' arguments; 2:2
*offers and defends* opinions; 2:1 does so *in a reasoned manner*; First does
so *persuasively*.

---

## 3. Universal rules (Essay, PB and GE)

Each rule gives what to do, what to avoid, and the evidence.

**U1. Answer the exact question set, from the first line to the last.**
Do: every section advances an answer to the precise statement, party or
outcome asked about. Avoid: a correct but question-neutral review of the
topic.

**U2. Take a clear position and defend it; do not sit on the fence.**
Do: state the conclusion, then show why the competing view is weaker. Avoid:
"likely", "clearly", "it depends" with no decision; hiding behind the jury.

**U3. Show the other side, then take a position.**
Do: give the strongest counterargument or alternative interpretation (a
dissent, a disputing academic, a narrower reading), then resolve it.

**U4. Support every material proposition with authority at the point it is made.**
Do: statute section or case (with pinpoint) beside the proposition; an
academic source beside every evaluative or empirical claim ("commentary
suggests", "regulatory failure", economic cost, statistics).

**U5. Use the controlling authority, and the highest court.**
Do: find the case that goes "to the heart" of the issue; cite the appellate
decision rather than the first-instance one; check it has not been overruled.
Order: UKSC, CA, High Court, then specialist tribunals and ombudsmen.

**U6. State the precise provision, not just the Act.**
Do: name the operative section, rule or paragraph that decides the point.

**U7. Represent authority precisely: ratio vs obiter, majority vs dissent, exact formulation.**

**U8. Use secondary sources to carry the analysis.**
Do: at least one academic view per major argument; group like-minded authors
and contrast opposing camps; draw on the module reading list. Avoid: case law
only.

**U9. Spend words on the issues that allow deep analysis; compress the easy ones.**
Do: give one or two sentences to uncontroversial preliminaries (e.g.
dominance, settled definitions); give the budget to disputed or complex issues.

**U10. Depth over breadth: fewer points, fully argued.**

**U11. Stay within the question and the syllabus; cut irrelevant material.**

**U12. Use the whole word budget.**
Do: land within 90–110% of the target.

**U13. Be precise with legal terms and facts.**
Do: use terms in their legal meaning ("intent", "assault", "embryo", "subsidy
type" vs "allocation method"); keep the facts right. Avoid internal
contradictions.

**U14. Explain, do not assert, the link between the evidence and the point.**

**U15. Clean, economical, academic prose.**
Do: clear, concise sentences (about 2 lines); short paragraphs (about 6 lines);
defined short forms (e.g. **Eligible Adult Dependant** ("**EAD**")); no
contractions, colloquialisms or flowery vocabulary; zero spelling, punctuation
and grammar errors. Avoid bullet lists inside academic argument.

---

## 4. Essay rules

**E1. Introduction = thesis plus roadmap, not description.**
Direct it at the question; define terms of art; state the position the essay
will defend and the 2–3 issues it uses. First person ("I shall argue") or
"It will be argued" are both acceptable. Avoid an opening that only narrates
cases, and bare roadmaps ("This essay will discuss…").

**E2. Define key terms early.**

**E3. Build the argument; synthesise rather than summarise.**
Each section tests the thesis: state the law, then analyse it (its purpose
against its operation, flaws and limitations, analogies, your own examples),
then show what that section contributes to the thesis. Compare authorities
against each other.

**E4. Engage context: economic, social and political pressures, data, rationale.**

**E5. Offer a persuasive, defended view on effectiveness and reform, with its trade-offs.**

**E6. Explain scope choices.**
If you focus on one area (e.g. tort cases), say why.

**E7. Conclusion answers the question and follows from the analysis.**

---

## 5. Problem question (PB) rules

**P1. Structure by party or issue, following the question's own structure.**
Use subheadings for each party or claim. Open each party with a one-line
finding.

**P2. Spot every issue, including separate interests hidden in the facts.**
Work through the facts line by line; each right, act or document is a
potential issue (e.g. the option to purchase, the consent agreement).

**P3. State each element of the full legal test, then apply each element to specific facts.**
Never apply a partial test; include the exceptions (e.g. Sch 3 para 2
exceptions; s29 LRA). Build towards the conclusion: element, how the courts
interpret it, the facts, a finding.

**P4. Apply, do not restate.**
Use the facts to argue, not to retell.

**P5. Identify missing or ambiguous facts, state a reasonable assumption, and follow it through consistently.**

**P6. Deploy the strongest route first; consider every materially available alternative route and remedy.**

**P7. Solve by elimination where the facts allow; it saves words.**

**P8. Distinguish or draw parallels with the facts of decided cases.**

**P9. No generic introductions or conclusions in PB; no "the next issue is".**
Go straight to the analysis; conclude per party. Do not explain that both
actus reus and mens rea are required.

**P10. Grade harm, liability and remedy against the specific standard.**

---

## 6. GE (general enquiry) rules — adapted

No module examines GE, so these rules adapt the PB and communication evidence
to a plain-language answer. They carry lower evidential weight than sections 3–5.

- **G1** Direct answer first (U1, U2), then the decisive conditions, then what to
  do next.
- **G2** State the full test for each route, with its conditions and deadlines
  (P3, U6).
- **G3** Name the missing facts that would change the outcome, give the
  assumption you are using, and ask only for facts that matter (P5).
- **G4** Put the strongest route first, then list alternatives only if they are
  material (P6, U11).
- **G5** Plain, precise language; every material legal sentence carries its
  citation (U4, U13, U15).

---

## 7. Citation and OSCOLA rules

**C1. In-text OSCOLA in brackets after the supported sentence** (owner
preference; Criminal lecture: "Use in text citations, not footnotes"). The host
renders citations from verified metadata. The model never writes them.

**C2. Pinpoint every case, article or book** to the paragraph (square brackets
for cases), or to the page where there are no paragraph numbers. Never copy a
pinpoint from a secondary source without checking it.

**C3. OSCOLA 5 (2026), not OSCOLA 4.** Key changes that affect the renderer:
- **no *ibid***; use short forms or cross-references (1.2.1, 1.2.3)
- no retrospective neutral citations for pre-2001 cases (1.1.1)
- court identifiers updated: KBD and QBD (2.1.5)
- guidance for assimilated EU law (2.4.9)
- ECLI format for EU court cases (4.4.2)
- generative AI citation (3.7.13)
- persistent links for websites (3.7.1)

The renderer in `backend/app/citations/oscola.py` needs an OSCOLA 5 check
(section 10).

**C4. Examples to match (OSCOLA 5 quick reference):**
- *Corr v IBC Vehicles Ltd* [2008] UKHL 13, [2008] 1 AC 884
- *Callery v Gray* [2001] EWCA Civ 1117, [2001] 1 WLR 2112 [42], [45]
- Human Rights Act 1998, s 15(1)(b)
- Paul Craig, 'Theory, "Pure Theory" and Values in Public Law' [2005] PL 440, 441
- Law Commission, *Reforming Bribery* (Law Com No 313, 2008) paras 3.12–3.17

**C5. Pensions Ombudsman determinations** (OSCOLA is silent; module rule):
*S Mr* [24696] Pensions Ombudsman Determination (30 December 2019), with only
the name italicised.

**C6. Bibliography only when asked;** list only sources actually cited, grouped
into cases, legislation and secondary sources.

**C7. Name authors plainly** ("Miller argues"), without titles; attribute every
view to its source.

---

## 8. Subject rules confirmed by markers

These are substantive points taken from marker comments. The four marked
**verified** were checked against the statute text in your `Official
Legislation` folder. The rest rest on marker authority; the verification pass
(section 10) will confirm their currency.

- **Land (registered land).**
  - An expressly granted legal easement over a registered estate is a
    registrable disposition (LRA 2002 s 27(2)(d)). Until registered it takes
    effect in equity only and **cannot override under Sch 3 para 3**, which
    covers legal easements only. **Verified.**
  - A lease of more than 7 years is a registrable disposition (s 27(2)(b)(i));
    unregistered, it is at best equitable, and may override only via Sch 3
    para 2 (actual occupation, subject to its exceptions). It cannot use
    para 1. **Verified.**
  - An option to purchase binds only if protected by a notice.
  - Contribution-based beneficial interests (*Boland*): check overreaching
    (LPA 1925 ss 2, 27) before actual occupation.
  - Proprietary estoppel: cover the presumption of reliance, and weigh benefits
    received (e.g. living rent-free).
- **Commercial / sale of goods.**
  - Property in specific goods not yet in a deliverable state passes under SGA
    1979 s 18 r 2 only when the thing is done and the buyer has notice.
    **Verified.**
  - Consider s 28 (payment and delivery are concurrent conditions).
  - For consumer risk, use CRA 2015 s 29: the trader bears risk until physical
    possession. **Verified.**
  - For nemo dat, cite the Factors Act 1889 alongside the SGA.
- **Trusts.**
  - Consider secret trusts where property passes on an agreement.
  - Distinguish fiduciary from non-fiduciary duties.
  - In tracing, consider subrogation for money used to discharge a debt, and
    knowing receipt.
- **Criminal (non-fatal offences).**
  - Work through AR, MR and defences per party.
  - Where D intended the wound, s 18 is live; then address consent (*Brown*;
    *Barnes* factors for sport).
  - Consent obtained by deception: *Tabassum*.
  - Assault requires V to apprehend: no awareness, no assault.
  - Transferred malice applies within the same offence only (*Latimer*).
  - Grade the harm using case law and the CPS charging standards.
- **Law & medicine.**
  - Advance refusal: existence (MCA 2005 s 24), validity (s 25(2)),
    applicability (s 25(4)), and the life-sustaining conditions (s 25(5)–(6)).
  - Best interests: s 4 and *Aintree v James*.
  - Capacity: distinguish the diagnostic and functional elements (*Re C*,
    *A Local Authority v JB*).
  - Children: Gillick competence, parental responsibility (Children Act 1989
    ss 2, 4, **verified**), and *Re W* as followed in *E v Northern Care
    Alliance*.
  - Organ donation: HTA 2004 s 1 appropriate consent; the s 3(6) hierarchy;
    s 32 on commercial dealings.
- **Pensions.**
  - Start from the scheme's purpose and PA 2004 s 255 (activities limited to
    retirement-benefit related activities).
  - *Palestine* was 3:2, and its comments on the Law Commission test are obiter;
    note the two formulations.
  - Edge (CA) at [50] on "impartiality".
  - Section 67 PA 1995 does not apply to unregistered schemes.
  - Financially interdependent persons reach the Ombudsman via reg 1A of the
    1996 Regulations.
  - For Women Scotland [2025] UKSC 16 (biological sex under the Equality Act
    2010).
  - British Airways (CA) on improper purpose.
- **Private international law.**
  - Consider comity in foreign-illegality cases.
  - Distinguish the *Ralli Bros* rule from the non-enforcement of foreign penal
    law; Rome I art 9(3) and public policy are alternative bases.
  - Engage Lord Leggatt's dissent in *Brownlie II*.
- **Competition.**
  - Keep dominance to one or two sentences unless it is contested; develop the
    abuses (e.g. exclusivity); support findings of abuse with secondary
    literature.
- **EU internal market.**
  - Engage *De Agostini* where it is central; look at the free movement of
    services where relevant.

---

## 9. Compact version for Qwen (fits in about 1,800 characters)

1. Answer the exact question. Take a clear, defended position; no fence-sitting.
2. Give the strongest counterargument or alternative reading, then resolve it.
3. Every legal proposition gets the precise provision or the controlling
   (highest) case; every evaluative claim gets an academic source.
4. Represent authority exactly: ratio or obiter, majority or dissent, exact
   wording.
5. Spend words where analysis is deep; one line for easy points. Fewer points,
   fully argued. Nothing outside the question.
6. Essay: thesis-led introduction; synthesise and compare; context and reform
   trade-offs; the conclusion answers the question.
7. PB: one section per party or issue; every element of the full test applied to
   specific facts; state assumptions for missing facts and stay consistent;
   strongest route first, then material alternatives and remedies; no generic
   introduction.
8. GE: direct answer, conditions, next steps, missing facts.
9. Precise legal terms; no contradictions; short, clear sentences; use about
   90–110% of the word target.

---

## 10. Follow-up work this guide creates

1. Encode sections 3–7 as the new sealed guidance bundle (replacing
   `owner-standards-2026-09-23.3`), and give Qwen either all rules (16k context)
   or section 9.
2. Update the drafting prompt: add P5 (assumptions), P6 (strongest route
   first), U3 (counterargument) and U7 (ratio or obiter precision).
3. Check `oscola.py` against OSCOLA 5: no *ibid*, KBD, ECLI, and the Pensions
   Ombudsman format.
4. Route section 8 into the subject-rule lane; the verification pass confirms
   the currency of each point.
5. Add reviewer checks for U2 (fence-sitting phrases), U11 (off-question
   content) and U13 (internal contradiction).

## 11. Conflicts between modules and how they are resolved

- **Introductions:** Criminal says no introduction in PB; L&M and the essay
  guidance require a thesis-led introduction in essays. Resolved by mode.
- **Numbering:** Pensions requires numbered headings; the general Pensions note
 and the dissertation markers reject numbering and bullets inside academic
  prose. Resolved: headings allowed; no numbered or bulleted argument in essays.
- **Footnotes vs in-text:** most modules use footnotes; Criminal and the owner
  prefer in-text. The owner's choice wins (C1).
