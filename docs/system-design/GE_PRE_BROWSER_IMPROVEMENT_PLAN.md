...
```mermaid
flowchart TD
    UI[Website chat and provider connections] --> API[Authenticated LegalBot API]
    API --> JOB[Durable job and frozen route]
    JOB --> RUN[AnswerRunner]
    RUN --> FACT[Conversation and matter facts]
    RUN --> RET[Scoped hybrid retrieval]
    GUIDE[Versioned guides and application policy] --> PACK[Evidence and instruction pack]
    FACT --> PACK
    RET --> PACK
    PACK --> ROUTE[Provider adapter]
    ROUTE --> Q[Own local Qwen]
    ROUTE --> L[Linked local model]
    ROUTE --> C[Local Codex bridge]
    ROUTE --> H[Hosted model API]
    Q --> DRAFT[Private structured draft]
    L --> DRAFT
    C --> DRAFT
    H --> DRAFT
    DRAFT --> REVIEW[Deterministic checks and independent review]
    REVIEW --> RELEASE[Atomic selected-chain publication or honest hold]
    RELEASE --> STORE[Persisted answer and event outbox]
    STORE --> UI
    INGEST[Source capture and proposition review] --> INDEX[Versioned index generation]
    INDEX --> RET
```
