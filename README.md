...
The single local chat application now connects Codex, base Qwen and per-session
OpenAI/Claude/Gemini API credentials to the existing durable backend. It saves
conversations and literal case facts, performs scoped retrieval and official
source research, and keeps raw model invocations and displayed transcripts in
encrypted local storage. Operations dashboard code has been removed; management
APIs are disabled by default.

**The legal-answering acceptance test has not passed.** The real browser campaign
submitted 20 cases per selected model and six withheld follow-ups: 40 conversations,
46 messages, zero substantive published answers. All final requests held for
missing qualifying evidence. The six clarification sequences resolved correctly.
Both Codex and base Qwen passed actual connection tests. These source holds do
not measure either model's legal accuracy or establish a 70+ writing score.

See [current state](docs/CURRENT_STATE.md), the
[implementation and acceptance report](docs/testing/SHARED_CHAT_2026-09-23.md),
[40-case machine-readable results](docs/testing/CHAT_CAMPAIGN_2026-09-23_RESULTS.json)
and the [maintained system design](docs/system-design/GE_PRE_BROWSER_IMPROVEMENT_PLAN.md).
Earlier standalone drafts and historical evaluations are not current UI passes.

## Start the local chat

After installing dependencies and obtaining a reviewed local retrieval manifest:

```bash
.venv/bin/python scripts/launch_session_chat.py \
  --state shared-chat-your-fresh-run \
  --retrieval-manifest /path/to/GE-QWEN-DEVELOPMENT-RETRIEVAL.json \
  --start-qwen
```

Use `--codex-model MODEL_ID` to explicitly choose a model available to the signed-in
Codex account. The selected model is pinned in the development authority and shown
in chat; no automatic model fallback occurs.

Open `http://127.0.0.1:8777/`. The launcher creates a fresh non-ACTIVE development
candidate and privately configures API/worker authority. Reusing an execution state
is refused. Stop the launcher with Ctrl-C to stop its owned process tree.

A public clone contains code and exposed test scenarios, not private source
captures, licensed text, model weights, credentials or reviewed vector generations.
It therefore cannot produce qualified legal answers immediately after cloning.
The current installed generation is England consumer law reviewed as at
5 September 2026; selecting another date or jurisdiction does not broaden that review.

Codex uses this Mac's signed-in CLI and a remote model (the launcher selects an
explicit account-available model; the current focused diagnostic selected
`gpt-6-astra`). Qwen runs locally. API connections require a key for the selected
provider; no hosted-provider live success is claimed without one. A website cannot
automatically use a visitor's own Codex or Claude desktop session.

## Evidence, rights and privacy

UK and USA are the first coverage targets; support is checked for each question.
Current online discovery is bounded, not a general deep-search engine. UK dated
legislation checks reject unresolved effects/version problems. US official pages
can be privately captured, but source/currentness review is required before use.
The owner supplied executed Find Case Law terms covering local parsing,
BM25/vector indexing and RAG for an England-and-Wales research assistant used
for private study and invited friends. The signed file is not yet bound to the
runtime permission gate. Current-version, withdrawal, personal-data, approved
notice and crawler controls are required before licensed judgments are served.
The stated purpose does not expressly describe external-model processing,
internet hosting or answer-weight training on that text. The current local chat
has no friend authentication, so an open internet link would not enforce the
intended audience.

The default citation style is OSCOLA. Requested bibliographies use deduplicated
Case law, Legislation and Secondary sources groups. GE/Essay/PB writing criteria
and the 70+ rubric do not override material evidence or factual defects.
No new training, ACTIVE change, protected-bank execution or public hosting is
part of the current test result. Conditional LoRA remains dependent on eligible
source-reviewed examples and demonstrated model-specific failures.

## Technical reference

The sections below describe retained local tools and contracts. Commands that
build or admit a source require their documented review gates; they are not
shortcuts around the current evidence holds.
...
Jurisdiction filtering uses both a broad typed class and an exact normalized key.
The existing `comparative` enum must not pool distinct jurisdictions or determine
current product scope. The UK/USA amendment requires precise location and issue-level
applicability; this documentation change does not establish runtime conformance.
