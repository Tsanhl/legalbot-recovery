...
def test_packets_bind_all_source_links_without_semantic_decision(tmp_path: Path) -> None:
    ...
    persisted = json.loads(
        (output / "DETERMINISTIC-SOURCE-REVIEW-PACKETS-26.json").read_bytes()
    )
...
def test_packets_expose_relevant_text_and_stale_locator(tmp_path: Path) -> None:
    ...
    by_link = {
        (row["row_id"], row["authority_identity_id"]): row
        for row in artifact["rows"]
    }

    undue = by_link[
        ("live30-q28:issue-05", "neutral-citation:[2009] EWHC 1076 (Ch)")
    ]
    assert undue["deterministic_diagnostics"][
        "exact_issue_phrase_occurrence_count"
    ] == 20
    assert any(
        "EXACT_ISSUE_PHRASE" in block["selection_reasons"]
        for block in undue["candidate_blocks"]
    )

    heard = by_link[
        ("live60-q34:issue-01", "neutral-citation:[2007] EWCA Crim 125")
    ]
    assert heard["source_title"] == "Heard, R. v"
    assert heard["deterministic_diagnostics"][
        "exact_issue_phrase_occurrence_count"
    ] == 0

    stale = by_link[
        ("live60-q33:issue-06", "neutral-citation:[2014] EWCA Civ 685")
    ]
    assert stale["deterministic_diagnostics"][
        "all_supplied_locators_resolved"
    ] is False
    ...
    getty = by_link[
        ("live30-q30:issue-16", "neutral-citation:[2025] EWHC 38 (Ch)")
    ]
    getty_paragraph_90 = next(
        block
        for block in getty["candidate_blocks"]
        if block["locator"] == "paragraph 90"
    )
...
def test_legislation_alias_is_preserved_without_new_admission(tmp_path: Path) -> None:
    artifact = packets.build_packets(tmp_path / "r104")
    rows = [
        row for row in artifact["rows"] if row["authority_identity_id"] == "ukpga:1957:31"
    ]
    assert len(rows) == 2
    assert {
        row["canonical_authority_identity_id"] for row in rows
    } == {"ukpga:Eliz2:5-6:31"}
