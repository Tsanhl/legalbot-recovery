...
def test_visible_and_private_packages_are_physically_separate() -> None:
    ...
    assert visible_manifest["status"] == "CORRECTED_VISIBLE_COMMON_PUBLIC_R2_READY_FOR_OWNER_REVIEW_NOT_PHASE2B"
    ...
    assert visible_manifest["private_unseen_package_content_sha256"] == private_manifest["package_content_sha256"]
...
def test_administrative_and_wills_are_draft_only_pending_source_admission() -> None:
    for root in (VISIBLE, PRIVATE):
        for topic_id in ("administrative-law", "wills-and-estates"):
            ...
            assert rows
            assert all(row["topic_execution_status"] == "DRAFT_ONLY_BLOCKED_PENDING_OFFICIAL_SOURCE_ADMISSION" for row in rows)
            assert all(row["scored_evaluation_eligible"] is False for row in rows)


def test_cross_bank_contamination_checks_pass() -> None:
    visible_audit = json.loads((VISIBLE / "CROSS-BANK-CONTAMINATION-AUDIT.json").read_text(encoding="utf-8"))
    private_audit = json.loads((PRIVATE / "CROSS-BANK-CONTAMINATION-AUDIT.json").read_text(encoding="utf-8"))
    assert visible_audit["exact_normalized_prompt_overlap_count"] == 0
    assert visible_audit["maximum_similarity"] < visible_audit["near_overlap_threshold_exclusive"] == 0.55
    assert private_audit["exact_normalized_prompt_overlap_count"] == 0
    assert private_audit["maximum_similarity"] < private_audit["near_overlap_threshold_exclusive"] == 0.55
