...
def test_review_plan_is_non_admitting_and_corrects_unrelated_mappings() -> None:
    ...
    assert sum(value["source_recommendation"].startswith("PROPOSE_") for value in values) == 4
    lifestyle = next(value for value in values if value["proposal_id"] == "issue-source-rehome-004")
    assert lifestyle["source_recommendation"] == ("REJECT_SOURCE_FOR_PLANNED_BENCHMARK_ROWS")
    assert lifestyle["supported_claims"] == []
    byers = next(value for value in values if value["proposal_id"] == "issue-source-rehome-003")
...
def test_build_verifies_exact_spans_and_preserves_owner_gate(tmp_path: Path) -> None:
    ...
    batch = review._load_object(tmp_path / "review/OWNER-ISSUE-SOURCE-ADMISSION-BATCH.json")
    assert (
        review._verify_seal(
            batch,
...
def test_build_rejects_mutated_exact_span(tmp_path: Path) -> None:
    ...
    plan["reviews"][0]["supported_claims"][0]["exact_normalized_span_text"] = (
        "Generally it is the doctor's consent which makes treatment lawful."
    )
    plan_path = tmp_path / "mutated-plan.json"
    plan_path.write_text(json.dumps(plan), encoding="utf-8")

    with pytest.raises(ValueError, match="phase2a_rehoming_review_exact_span_not_unique"):
        review.build(
            plan_path=plan_path,
            quarantine_root=review.DEFAULT_QUARANTINE_ROOT,
