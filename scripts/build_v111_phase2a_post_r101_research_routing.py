...
DEFAULT_OUTPUT_ROOT = (
    REVIEW_ROOT / "LegalBot-Phase2AB-2026-08-26-r102-post-r101-research-routing"
)
...
def _records(
    value: Mapping[str, Any], key: str, expected: int, code: str
) -> list[dict[str, Any]]:
...
def _route_row(
    *,
    ...
    original_outside = [
        str(item)
        for item in recovery.get("planned_authority_ids_outside_candidate", [])
    ]
    effective_outside = [
        item for item in original_outside if item not in rejected_authorities
    ]
    ...
    if original_outside and all(
        item in rejected_authorities for item in original_outside
    ):
...
def build_routing(output_root: Path = DEFAULT_OUTPUT_ROOT) -> dict[str, Any]:
    ...
    ready = _records(
        ready_artifact, "records", 3, "phase2a_r102_ready_rows_invalid"
    )
    retained_rows = _records(
        retained_artifact, "records", 5, "phase2a_r102_retained_rows_invalid"
    )
    ...
    if (
        len(by_recovery) != 361
        or len(by_triage) != 448
        or len(by_retained) != 5
    ):
    ...
    approved_rejection_hashes = {
        ...
        for item in decision["source_decision"][
    ...
    for finding in findings:
        ...
        material = {
            ...
            "remaining_dependency": (
                retained.get("remaining_dependency") if retained else route
            ),
    ...
    if len(outside_records) != 16 or sum(
    ...
    material = {
        ...
        "source_r96_ready_content_sha256": ready_artifact[
            "artifact_content_sha256"
        ],
        "source_r96_retained_content_sha256": retained_artifact[
            "artifact_content_sha256"
        ],
        "source_r71_content_sha256": r71["artifact_content_sha256"],
        "source_r86_content_sha256": r86["artifact_content_sha256"],
        "source_r95_uksc_content_sha256": r95_uksc[
            "artifact_content_sha256"
        ],
