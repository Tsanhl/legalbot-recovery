#!/usr/bin/env python3
"""Evaluate the planned-and-audited route on a third new 23-domain set."""
...
CAMPAIGN_ID = "LegalBot-GE-2026-09-04-post-unseen-fresh-audited-route-r1"
...
SOURCES = (
    Source("administrative-law", "LEGAL_TOPIC", "lga26", "Local Government Act 1974", "ukpga/1974/7/section/26", "section 26"),
    Source("ai-and-data-protection", "LEGAL_TOPIC", "dea40", "Digital Economy Act 2017", "ukpga/2017/30/section/40", "section 40"),
    Source("business-and-company-law", "LEGAL_TOPIC", "ccbsa2", "Co-operative and Community Benefit Societies Act 2014", "ukpga/2014/14/section/2", "section 2"),
    Source("commercial-law", "LEGAL_TOPIC", "factors2", "Factors Act 1889", "ukpga/Vict/52-53/45/section/2", "section 2"),
    Source("competition-law", "LEGAL_TOPIC", "nsia13", "National Security and Investment Act 2021", "ukpga/2021/25/section/13", "section 13"),
    Source("contemporary-biolaw-and-regulation", "LEGAL_TOPIC", "aspa3", "Animals (Scientific Procedures) Act 1986", "ukpga/1986/14/section/3", "section 3"),
    Source("contract-law", "LEGAL_TOPIC", "cida2", "Consumer Insurance (Disclosure and Representations) Act 2012", "ukpga/2012/6/section/2", "section 2"),
    Source("criminal-law", "LEGAL_TOPIC", "sca44", "Serious Crime Act 2007", "ukpga/2007/27/section/44", "section 44"),
    Source("eu-internal-market-law", "LEGAL_TOPIC", "euwaa5", "European Union (Withdrawal Agreement) Act 2020", "ukpga/2020/1/section/5", "section 5"),
    Source("international-commercial-mediation", "LEGAL_TOPIC", "cbmeu5", "The Cross-Border Mediation (EU Directive) (EU Exit) Regulations 2019", "uksi/2019/469/regulation/5", "regulation 5"),
    Source("land-law", "LEGAL_TOPIC", "lrgra3", "Leasehold Reform (Ground Rent) Act 2022", "ukpga/2022/1/section/3", "section 3"),
    Source("law-and-medicine", "LEGAL_TOPIC", "hsca20", "Health and Social Care Act 2008", "ukpga/2008/14/section/20", "section 20"),
    Source("pensions-law", "LEGAL_TOPIC", "pa1995_47", "Pensions Act 1995", "ukpga/1995/26/section/47", "section 47"),
    Source("private-international-law", "LEGAL_TOPIC", "pila11", "Private International Law (Miscellaneous Provisions) Act 1995", "ukpga/1995/42/section/11", "section 11"),
    Source("tort-law", "LEGAL_TOPIC", "faa1", "Fatal Accidents Act 1976", "ukpga/1976/30/section/1", "section 1"),
    Source("trusts-law", "LEGAL_TOPIC", "vta1", "Variation of Trusts Act 1958", "ukpga/Eliz2/6-7/53/section/1", "section 1"),
    Source("wills-and-estates", "LEGAL_TOPIC", "fa1982_2", "Forfeiture Act 1982", "ukpga/1982/34/section/2", "section 2"),
    Source("housing", "PUBLIC_ACCESS_DOMAIN", "hra2017_1", "Homelessness Reduction Act 2017", "ukpga/2017/13/section/1", "section 1"),
    Source("employment", "PUBLIC_ACCESS_DOMAIN", "tulr188", "Trade Union and Labour Relations (Consolidation) Act 1992", "ukpga/1992/52/section/188", "section 188"),
    Source("family", "PUBLIC_ACCESS_DOMAIN", "ddsa1", "Divorce, Dissolution and Separation Act 2020", "ukpga/2020/11/section/1", "section 1"),
    Source("immigration", "PUBLIC_ACCESS_DOMAIN", "iaa95", "Immigration and Asylum Act 1999", "ukpga/1999/33/section/95", "section 95"),
    Source("benefits-and-debt", "PUBLIC_ACCESS_DOMAIN", "csa1", "Child Support Act 1991", "ukpga/1991/48/section/1", "section 1"),
    Source("consumer", "PUBLIC_ACCESS_DOMAIN", "timeshare14", "The Timeshare, Holiday Products, Resale and Exchange Contracts Regulations 2010", "uksi/2010/2960/regulation/14", "regulation 14"),
)
...
AUDIT_PROMPT = """Read input.json only. Audit each draft against its question, official evidence and independent coverage plan before freezing it. Check that every planned condition and exception is stated accurately, no unsupported narrowing or addition appears, all unverified facts and limits remain qualified, and the practical or urgent step is proportionate. Return a final answer for every assigned case. Preserve a compliant draft exactly; otherwise make only the changes needed to cure the source-and-plan defects. Do not browse, run code, inspect other paths, mention the audit, or claim professional review."""
...
class PlannedRouteError(RuntimeError):
    """The new planned-and-audited route violated its contract."""
...
def run_stage(stage: str) -> dict[str, Any]:
    prompts = {"author": AUTHOR_PROMPT, "plan": PLAN_PROMPT, "answer": ANSWER_PROMPT, "audit": AUDIT_PROMPT, "review": REVIEW_PROMPT}
    root = SESSION_ROOT / stage
...
def prepare() -> dict[str, Any]:
    if PUBLIC_ROOT.exists() or SESSION_ROOT.exists():
        raise PlannedRouteError("refusing to replace existing audited-route campaign")
...
def accept_authors() -> dict[str, Any]:
    ...
    expected = {f"{source.topic}:post-unseen-fresh-audit-r1-01" for source in SOURCES}
    ...
    for ordinal, source_spec in enumerate(SOURCES, 1):
        case_id = f"{source_spec.topic}:post-unseen-fresh-audit-r1-01"
        ...
        cases.append(sealed({"schema": "legalbot.ge-post-unseen-fresh-audited-case.v1", "ordinal": ordinal, "case_id": case_id, "topic": source_spec.topic, "coverage_kind": source_spec.coverage_kind, "question": question, "question_hash": question_hash, "evidence_references": [reference], "private_unseen_source": False}))
        controls.append(sealed({"schema": "legalbot.ge-post-unseen-fresh-audited-control.v1", "ordinal": ordinal, "case_id": case_id, "required_points": required, "material_limits": limits, "prohibited_overclaims": overclaims, "evidence_references": [reference]}))
...
def accept_plans() -> dict[str, Any]:
    ...
    for case_id in sorted(cases, key=lambda key: cases[key]["ordinal"]):
        ...
        locator_matches = (
            echoed_locator.lower() == canonical_locator.lower()
            or echoed_locator.lower().startswith(canonical_locator.lower() + ";")
            or echoed_locator.lower().startswith(
                str(source["title"]).lower() + ", " + canonical_locator.lower() + ";"
            )
        )
...
def accept_answers() -> dict[str, Any]:
    ...
    cases = {row["case_id"]: row for row in load_jsonl(PUBLIC_ROOT / "FRESH-VISIBLE-CASES.jsonl")}
    plans = {row["case_id"]: row for row in load_jsonl(PUBLIC_ROOT / "ANSWER-COVERAGE-PLANS.jsonl")}
    sources = {row["topic"]: row for row in load_jsonl(PUBLIC_ROOT / "OFFICIAL-SOURCE-MANIFEST.jsonl")}
    ...
    for case_id in sorted(cases, key=lambda key: cases[key]["ordinal"]):
        ...
        drafts.append(sealed({"schema": "legalbot.ge-post-unseen-fresh-audited-draft.v1", "case_id": case_id, "question_hash": cases[case_id]["question_hash"], "draft_answer": answer, "draft_answer_hash": answer_hash, "candidate_variant": "CODEX_SOURCE_BOUND_PLANNED_DRAFT", "adapter_used": False, "weight_training_performed": False, "professional_legal_sign_off": False}))
        source = sources[cases[case_id]["topic"]]
        plan = plans[case_id]
        audit_rows.append({"case_id": case_id, "question": cases[case_id]["question"], "draft_answer": answer, "draft_answer_hash": answer_hash, "coverage_plan": {key: plan[key] for key in ("direct_answer", "material_conditions_and_exceptions", "unverified_facts", "jurisdiction_and_currentness_limits", "source_name", "locator", "safe_practical_next_step", "urgent_safety_wording_needed")}, "official_evidence": {key: source[key] for key in ("title", "locator", "canonical_url", "document_status", "consolidated_valid_date", "metadata_modified_date", "raw_sha256", "evidence_span_sha256", "evidence_excerpt")}})
    write_jsonl(PUBLIC_ROOT / "PLANNED-DRAFT-OUTPUTS.jsonl", drafts)
    make_shards("audit", audit_rows, answer_schema(), "answers")
    return {"stage": "DRAFTS_ACCEPTED", "draft_count": len(drafts)}


def accept_audits() -> dict[str, Any]:
    audited = collect("audit", "answers")
    cases = {row["case_id"]: row for row in load_jsonl(PUBLIC_ROOT / "FRESH-VISIBLE-CASES.jsonl")}
    controls = {row["case_id"]: row for row in load_jsonl(PUBLIC_ROOT / "EXPECTED-CONTROLS.jsonl")}
    sources = {row["topic"]: row for row in load_jsonl(PUBLIC_ROOT / "OFFICIAL-SOURCE-MANIFEST.jsonl")}
    drafts = {row["case_id"]: row for row in load_jsonl(PUBLIC_ROOT / "PLANNED-DRAFT-OUTPUTS.jsonl")}
    if len(audited) != 23 or {row.get("case_id") for row in audited} != set(cases):
        raise PlannedRouteError("audit output case set mismatch")
    outputs = []
    reviews = []
    audit_register = []
    for case_id in sorted(cases, key=lambda key: cases[key]["ordinal"]):
        answer = normalise(str(next(row["candidate_answer"] for row in audited if row["case_id"] == case_id)))
        if not 70 <= len(answer.split()) <= 520 or any(
            token in answer.lower()
            for token in ("candidate answer", "owner review", "coverage plan", "audit stage")
        ):
            raise PlannedRouteError(f"audited answer surface failed: {case_id}")
        answer_hash = sha256_bytes(answer.encode())
        changed = answer_hash != drafts[case_id]["draft_answer_hash"]
        outputs.append(sealed({"schema": "legalbot.ge-post-unseen-fresh-audited-output.v1", "case_id": case_id, "question_hash": cases[case_id]["question_hash"], "candidate_answer": answer, "candidate_answer_hash": answer_hash, "candidate_variant": "CODEX_SOURCE_BOUND_PLANNED_AND_AUDITED_NON_WEIGHT", "draft_answer_hash": drafts[case_id]["draft_answer_hash"], "audit_changed_answer": changed, "adapter_used": False, "weight_training_performed": False, "professional_legal_sign_off": False}))
        audit_register.append(sealed({"schema": "legalbot.ge-post-unseen-source-plan-audit-receipt.v1", "case_id": case_id, "draft_answer_hash": drafts[case_id]["draft_answer_hash"], "final_answer_hash": answer_hash, "answer_changed": changed, "evaluator_controls_seen": False}))
        source = sources[cases[case_id]["topic"]]
    ...
    write_jsonl(PUBLIC_ROOT / "AUDITED-CODEX-OUTPUTS.jsonl", outputs)
    write_jsonl(PUBLIC_ROOT / "SOURCE-PLAN-AUDIT-REGISTER.jsonl", audit_register)
    make_shards("review", reviews, review_schema(), "reviews")
    return {"stage": "AUDITED_ANSWERS_ACCEPTED", "answer_count": len(outputs), "audit_changed_count": sum(bool(row["answer_changed"]) for row in audit_register)}


def finalize() -> dict[str, Any]:
    cases = {row["case_id"]: row for row in load_jsonl(PUBLIC_ROOT / "FRESH-VISIBLE-CASES.jsonl")}
    controls = {row["case_id"]: row for row in load_jsonl(PUBLIC_ROOT / "EXPECTED-CONTROLS.jsonl")}
    outputs = {row["case_id"]: row for row in load_jsonl(PUBLIC_ROOT / "AUDITED-CODEX-OUTPUTS.jsonl")}
    ...
    for case_id in sorted(cases, key=lambda key: cases[key]["ordinal"]):
        ...
        if row["candidate_answer_hash"] != expected_hash:
            corrections.append(sealed({"schema": "legalbot.ge-post-unseen-fresh-audited-review-hash-correction.v1", "case_id": case_id, "reviewer_echoed_hash": row["candidate_answer_hash"], "bound_hash": expected_hash, "answer_changed": False, "review_changed": False}))
    ...
    audit_register = load_jsonl(PUBLIC_ROOT / "SOURCE-PLAN-AUDIT-REGISTER.jsonl")
    metrics = sealed({"schema": "legalbot.ge-post-unseen-fresh-audited-metrics.v1", "campaign_id": CAMPAIGN_ID, "case_count": 23, "domain_count": 23, "source_count": 23, "coverage_plan_count": 23, "source_plan_audit_count": 23, "audit_changed_answer_count": sum(bool(row["answer_changed"]) for row in audit_register), "factual_pass_count": factual, "factual_hold_count": 23 - factual, "quality_70_floor_pass_count": full, "quality_hold_count": 23 - full, "mean_quality_score_on_factual_pass": round(sum(scores) / len(scores), 2) if scores else None, "minimum_quality_score_on_factual_pass": min(scores) if scores else None, "declared_material_claim_count": claims, "reviewed_material_claim_count": claims, "review_coverage_pct": 100.0, "failed_case_ids": failed, "mechanical_hash_correction_count": len(corrections), "route_pass": route_pass, "pass_definition": "23/23 factual and 23/23 score at least 70 with all critical floors."})
    write_json(PUBLIC_ROOT / "METRIC-REPORT.json", metrics)
    state_name = "POST_UNSEEN_FRESH_AUDITED_ROUTE_PASS_AWAITING_NEW_UNSEEN_BANK_DESIGN" if route_pass else "POST_UNSEEN_FRESH_AUDITED_ROUTE_HOLD_STOP_NO_FURTHER_VISIBLE_LOOP"
    state = sealed({"schema": "legalbot.ge-post-unseen-fresh-audited-state.v1", "campaign_id": CAMPAIGN_ID, "overall_state": state_name, "fresh_visible_generalisation": "PASS" if route_pass else "HOLD", "consumed_private_306_bank": "OPENED_ONCE_RETIRED_NOT_ACCESSED", "consumed_unseen_prompt_or_findings_used": False, "new_unseen_bank": "NOT_CREATED", "new_unseen_execution": "NOT_AUTHORIZED_NOT_STARTED", "answer_weight_training": "NOT_AUTHORIZED_NOT_PERFORMED", "r2_adapter": "INACTIVE_NOT_USED", "qualified_legal_review": "NOT_STARTED", "answer_legal_gold": "NOT_STARTED", "promotion": "NOT_STARTED", "live": "NOT_STARTED", "next_gate": "DESIGN_AND_INDEPENDENT_CUSTODY_OF_NEW_UNSEEN_BANK" if route_pass else "STOP_AND_REASSESS_ROUTE", "completed_at": datetime.now(UTC).isoformat().replace("+00:00", "Z")})
    write_json(PUBLIC_ROOT / "STATE-TRANSITION-RECEIPT.json", state)
    (PUBLIC_ROOT / "README.md").write_text(f"# {CAMPAIGN_ID}\n\nThird new source-first visible evaluation. Separate ephemeral contexts authored 23 cases, planned answer coverage without evaluator controls, drafted answers, audited each draft against only its source and plan, and performed blind review without the plans.\n\n- Factual pass: {factual}/23\n- Full 70+/floor pass: {full}/23\n- Drafts changed by source-plan audit: {metrics['audit_changed_answer_count']}/23\n- Declared claims reviewed: {claims}/{claims}\n- State: `{state_name}`\n\nThis is same-provider AI evidence, not qualified legal review, professional assurance or legal gold. No training or adapter use occurred. The consumed unseen bank and its findings were not accessed.\n")
    manifest = sealed({"schema": "legalbot.ge-post-unseen-fresh-audited-run.v1", "campaign_id": CAMPAIGN_ID, "source_manifest_sha256": sha256_file(PUBLIC_ROOT / "OFFICIAL-SOURCE-MANIFEST.jsonl"), "cases_sha256": sha256_file(PUBLIC_ROOT / "FRESH-VISIBLE-CASES.jsonl"), "controls_sha256": sha256_file(PUBLIC_ROOT / "EXPECTED-CONTROLS.jsonl"), "plans_sha256": sha256_file(PUBLIC_ROOT / "ANSWER-COVERAGE-PLANS.jsonl"), "drafts_sha256": sha256_file(PUBLIC_ROOT / "PLANNED-DRAFT-OUTPUTS.jsonl"), "audit_register_sha256": sha256_file(PUBLIC_ROOT / "SOURCE-PLAN-AUDIT-REGISTER.jsonl"), "outputs_sha256": sha256_file(PUBLIC_ROOT / "AUDITED-CODEX-OUTPUTS.jsonl"), "reviews_sha256": sha256_file(PUBLIC_ROOT / "CODEX-BLIND-REVIEW.jsonl"), "metrics_sha256": sha256_file(PUBLIC_ROOT / "METRIC-REPORT.json"), "state_sha256": sha256_file(PUBLIC_ROOT / "STATE-TRANSITION-RECEIPT.json"), "implementation_sha256": sha256_file(Path(__file__)), "model": "gpt-5.6-sol", "reasoning_effort": "high", "same_provider_ai": True, "ephemeral_contexts": 30, "consumed_unseen_root_read_denied": True})
    ...
    write_json(PUBLIC_ROOT / "ARTIFACT-SHA256-REGISTER.json", {"schema": "legalbot.ge-post-unseen-fresh-audited-artifacts.v1", "campaign_id": CAMPAIGN_ID, "artifacts": artifacts})
...
def main() -> int:
    ...
    parser.add_argument("stage", choices=("prepare", "run-author", "accept-author", "run-plan", "accept-plan", "run-answer", "accept-answer", "run-audit", "accept-audit", "run-review", "finalize", "verify"))
    args = parser.parse_args()
    actions = {"prepare": prepare, "run-author": lambda: run_stage("author"), "accept-author": accept_authors, "run-plan": lambda: run_stage("plan"), "accept-plan": accept_plans, "run-answer": lambda: run_stage("answer"), "accept-answer": accept_answers, "run-audit": lambda: run_stage("audit"), "accept-audit": accept_audits, "run-review": lambda: run_stage("review"), "finalize": finalize, "verify": verify}
