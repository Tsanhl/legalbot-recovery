#!/usr/bin/env python3
"""Claim and run exactly attempt 2 of the retained recovery-b index build."""
...
WORKER_ID = "phase2a-recovery-b-index-worker-20260901-b"
...
def main() -> None:
    ...
    try:
        ...
        if (
            ...
            raise ValueError("exact recovery-b build is not queued for bounded attempt 2")
        ...
        if row is None:
            raise ValueError("exact recovery-b attempt 2 could not be claimed")
        if (
            row["id"] != JOB_ID
        ...
        print(
            json.dumps(
                {
                    "status": "CLAIMED_EXACT_RECOVERY_B_ATTEMPT_2",
                    "job_id": JOB_ID,
                    "build_id": BUILD_ID,
                    "attempt_count": 2,
                    "worker_id": WORKER_ID,
