...
DEFAULT_OUTPUT_ROOT = REVIEW_ROOT / "LegalBot-Phase2A-2026-08-29-finite-remediation-owner-approved-r1"
...
EXPECTED_CONTRACTS_CONTENT_SHA256 = "6cebd8b70b3044b0b4533c1036c3aaec1b8ac88c4a43873a585616bc7cd573bf"
...
EXPECTED_PACKET_CHECKSUMS_FILE_SHA256 = "a58919e89c6fdd71f2aafac0e739752d48e879b3eb26a65ee122055c6ae8b810"
EXPECTED_AUTHORITY_CONTENT_SHA256 = "eb0eda2f34c8b261ea38fc9d697257cdd3bd6253c18c2d91355328c8cb78ef7b"
...
EXPECTED_SIGNATURE_FIELDS_CONTENT_SHA256 = "72270681fd96c78cbdb73554b27f2ce35e0dc559d1ec08ae6f24bcac85624637"
EXPECTED_R2_PACKET_CONTENT_SHA256 = "9a84056b160f953f9073809c3519e22317370d04ec30887d0aeb20d2ca24e739"
EXPECTED_R2_CONTRACTS_CONTENT_SHA256 = "65c08285f9a35e0274f9ea2e621c8a8c45c02ffd14ad942f392df85f06765f5f"
EXPECTED_R1_PACKET_CONTENT_SHA256 = "e8e2cd116510d75c3afa64263f4d032ece563f2cd7aa0fd949cefe6538e59844"
EXPECTED_R1_CONTRACTS_CONTENT_SHA256 = "97483887e654a76a2c326bde5c730b0afe9ad9c217965fe34b849ab746e81e40"

STATUS = "R3_OWNER_ADOPTION_RECORDED_EXECUTION_CHAIN_AVAILABLE"
_SHA256_RE = re.compile(r"^[0-9a-f]{64}$")

_PROHIBITED_FALSE_FLAGS = (
    "answer_model_authorized", "answer_model_run", "answer_release_authorized",
    "answer_release_run", "answer_released", "phase2b_authorized", "phase2b_run",
    "development30_authorized", "development30_run", "owner_certification60_authorized",
    "owner_certification60_run", "o04_authorized", "o04_run", "validation30_authorized",
    "validation30_run", "validation30_unsealed", "promotion_authorized", "promotion_run",
    "active_pointer_write_authorized", "active_pointer_written",
    "previous_pointer_write_authorized", "previous_pointer_written",
    "live_activation_authorized", "live_activation_run", "training_export_authorized",
    "training_export_run",
)
_NOT_RUN_FALSE_FLAGS = (
    "owner_decisions_applied", "owner_outcomes_applied", "source_admitted",
    "source_scan_run", "successor_build_run", "index_built", "embedding_run",
    "retrieval_reattestation_run", "all585_qualification_run", "candidate_mutated",
    "catalogue_mutated", "technical_qualification_assigned",
)
...
def _verify_inputs(packet_root: Path, authority_root: Path) -> tuple[dict[str, Any], dict[str, Any], bytes]:
    expected_inventory = {
        PACKET_NAME, CONTRACTS_NAME, PROMPT_NAME, PACKET_PACKAGE_NAME, PACKET_CHECKSUMS_NAME
    }
    if packet_root.is_symlink() or not packet_root.is_dir() or {p.name for p in packet_root.iterdir()} != expected_inventory:
        raise ValueError("finite_adoption_packet_inventory_invalid")
    packet = _load_exact(packet_root / PACKET_NAME, EXPECTED_PACKET_FILE_SHA256, EXPECTED_PACKET_CONTENT_SHA256, code="finite_adoption_packet")
    contracts = _load_exact(packet_root / CONTRACTS_NAME, EXPECTED_CONTRACTS_FILE_SHA256, EXPECTED_CONTRACTS_CONTENT_SHA256, code="finite_adoption_contracts")
    package = _load_exact(packet_root / PACKET_PACKAGE_NAME, EXPECTED_PACKAGE_FILE_SHA256, EXPECTED_PACKAGE_CONTENT_SHA256, code="finite_adoption_package")
    for path, expected, code in (
        (packet_root / PROMPT_NAME, EXPECTED_PROMPT_FILE_SHA256, "finite_adoption_prompt"),
        (packet_root / PACKET_CHECKSUMS_NAME, EXPECTED_PACKET_CHECKSUMS_FILE_SHA256, "finite_adoption_checksums"),
    ):
    ...
    if packet.get("supersedes_r2_packet_content_sha256") != EXPECTED_R2_PACKET_CONTENT_SHA256 or packet.get("also_supersedes_unapproved_r1_packet_content_sha256") != EXPECTED_R1_PACKET_CONTENT_SHA256:
        raise ValueError("finite_adoption_packet_supersession_invalid")
    if contracts.get("supersedes_r2_contracts_content_sha256") != EXPECTED_R2_CONTRACTS_CONTENT_SHA256 or contracts.get("also_supersedes_unapproved_r1_contracts_content_sha256") != EXPECTED_R1_CONTRACTS_CONTENT_SHA256:
        raise ValueError("finite_adoption_contract_supersession_invalid")
    if packet.get("counts", {}).get("prequalification_blocker_row_count") != 146 or packet.get("counts", {}).get("exact_cohort_remediation_row_count") != 17 or packet.get("counts", {}).get("strict_human_review_handoff_row_count") != 129:
        raise ValueError("finite_adoption_counts_invalid")
    if len(contracts.get("source_decisions", [])) != 25 or len(contracts.get("row_outcomes", [])) != 146:
    ...
    authority = _load_exact(authority_root / AUTHORITY_NAME, EXPECTED_AUTHORITY_FILE_SHA256, EXPECTED_AUTHORITY_CONTENT_SHA256, code="finite_adoption_authority")
    required = {
        "chain_id": EXPECTED_CHAIN_ID, "status": "AVAILABLE_UNSPENT",
        "total_execution_chain_count": 1, "execution_chain_consumed_count": 0,
        "execution_chain_remaining_count": 1, "new_or_additional_authority_created": False,
    }
...
def record_owner_adoption(
    *,
    ...
    allowed = {
        "owner_decision_application_authorized": True, "source_admission_authorized": True,
        "complete_source_scan_authorized": True, "successor_build_authorized": True,
        "index_build_authorized": True, "embedding_authorized": True,
        "retrieval_reattestation_authorized": True, "all585_qualification_authorized": True,
    }
    base = {
        "status": STATUS,
        "owner_approved": True, "owner_adoption_recorded": True,
        "owner_typed_name": owner_typed_name, "owner_decision_date": owner_decision_date,
        "recorded_at": timestamp.isoformat(), "signature_evidence_mode": "SEPARATE_FOLLOWUP_MESSAGE",
        ...
        "exact_row_contract_count": 146, "exact_cohort_remediation_row_count": 17,
        "strict_human_review_handoff_row_count": 129, "exact_source_decision_count": 25,
        ...
        "execution_chain_id": EXPECTED_CHAIN_ID, "execution_chain_count": 1,
        "execution_chain_consumed_count": 0, "execution_chain_remaining_count": 1,
        "execution_chain_status": "AVAILABLE_UNSPENT",
        "new_or_additional_execution_authority_created": False,
        "technical_success_predeclared": False, "successor_must_remain_non_active": True,
        "successor_must_remain_answer_ineligible": True, **allowed,
        **{key: False for key in _NOT_RUN_FALSE_FLAGS},
        **{key: False for key in _PROHIBITED_FALSE_FLAGS},
    }
    receipt = _seal({"schema": "legalbot.v111.phase2a.finite-remediation-r3-owner-adoption-receipt.v1", **base})
    outcome = _seal({"schema": "legalbot.v111.phase2a.finite-remediation-r3-owner-adoption-outcome.v1", **base,
    ...
    authority_link = _seal({
        ...
        "total_count": 1, "consumed_count": 0, "remaining_count": 1,
        "new_or_additional_execution_authority_created": False,
...
def main() -> int:
    ...
    print(json.dumps({"status": receipt["status"], "receipt_content_sha256": receipt["artifact_content_sha256"], "output_root": args.output_root.name}, sort_keys=True))
    return 0
