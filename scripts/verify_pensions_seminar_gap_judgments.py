#!/usr/bin/env python3
"""Verify staged Pensions seminar-gap judgments without admitting them."""
...
def _load_object(path: Path) -> dict[str, Any]:
    if path.is_symlink() or not path.is_file():
        raise ValueError("judgment_verification_input_must_be_regular_file")
    value = json.loads(path.read_bytes())
    if not isinstance(value, dict):
        raise ValueError("judgment_verification_input_must_be_object")
    return value
...
def _validate_boundary(manifest: Mapping[str, Any]) -> None:
    if manifest.get("schema") != EXPECTED_SCHEMA:
        raise ValueError("judgment_manifest_schema_invalid")
    required_false = (
    ...
    if any(manifest.get(field) is not False for field in required_false):
        raise ValueError("judgment_manifest_authority_boundary_invalid")
    if not isinstance(manifest.get("targets"), list) or not manifest["targets"]:
        raise ValueError("judgment_manifest_target_inventory_invalid")


def _expected_jurisdiction(citation: str) -> str:
    if any(court in citation for court in ("UKSC", "UKPC", "UKHL")):
...
def _xml_identity(raw: bytes) -> tuple[str, str, str]:
    ...
    if b"<!DOCTYPE" in opening or b"<!ENTITY" in opening:
        raise ValueError("judgment_xml_unsafe_declaration")
    root = ET.fromstring(raw)
    if _local_name(root.tag) != "akomaNtoso":
        raise ValueError("judgment_xml_root_invalid")
    ...
    title = _attr(_first(work, "FRBRname"), "value").strip()
    citation = _text(_first(proprietary, "cite"))
    uri = _attr(_first(expression, "FRBRthis"), "value").strip()
...
def verify(
    *,
    ...
    parent_expected = manifest["parent_legislation_plan"]["file_sha256"]
    if _sha256(parent_bytes) != parent_expected:
        raise ValueError("parent_legislation_plan_hash_mismatch")
    ...
    try:
        ...
        for target in manifest["targets"]:
            ...
            if (
                ...
                raise ValueError("judgment_manifest_target_invalid")
            ...
            checks = {
                ...
                and bool(parsed_paragraphs),
                "runtime_parser_anchors_official": bool(parsed_paragraphs)
                and all(
                    str(block.source_anchor or "").startswith(expected_uri)
        ...
        report: dict[str, Any] = {
            ...
            "parent_legislation_plan_file_sha256": parent_expected,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Technically verify staged Pensions seminar-gap judgments."
    )
    parser.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST)
    parser.add_argument("--parent-plan", type=Path, default=DEFAULT_PARENT_PLAN)
